...
int main(int argc, char** argv) {
    ...
    while (!terminate) {
        ...
        close_client(&client);
      	/* Ya hemos gestionado al cliente, podemos olvidarnos de él */     
...
void handle_connection(Server server, Client client) {
    ...
    strcpy(message, "hola");
    if ((sent_bytes = send(client.socket, message, strlen(message), 0)) < 0) {
    ...
    if ((sent_bytes = send(client.socket, message, strlen(message) + 1, 0)) < 0) {
      perror("No se pudo enviar el mensaje");
      exit(EXIT_FAILURE);
    }
